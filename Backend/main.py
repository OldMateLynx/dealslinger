...
=======
JINA_API_KEY = os.getenv("JINA_AI_READER_API_KEY")
...
ANCHOR_FIELD_MASK = PLACE_FIELD_MASK + ",places.websiteUri"
...
async def geocode_business(client: httpx.AsyncClient, name: str) -> dict:
    ...
    resp = await client.post(
        ...
        headers={
            ...
            "X-Goog-FieldMask": ANCHOR_FIELD_MASK,
        },
        json={"textQuery": name},
    )
    data = resp.json()
...
async def jina_fetch_website_markdown(client: httpx.AsyncClient, url: str) -> str:
    headers = {}
    if JINA_API_KEY:
        headers["Authorization"] = f"Bearer {JINA_API_KEY}"

    resp = await client.get(f"{JINA_READER_BASE_URL}{url}", headers=headers, timeout=30.0)
    resp.raise_for_status()
    return resp.text


ENTIRE_PRODUCT_SEARCH_KEYWORDS: dict[str, tuple[str, ...]] = { #nav link searching for first scan
    "Skateboards": ("skate", "skateboard"), #then search "complete", if no complete nav link just skate/skateboard
    "Scooters": ("scooter",),
    "Knee_ElbowPads": ("pad", "protective gear", "protection", "protective"), #if not dedicated pad
    "Skate_Scooter_Bike_Helmets": ("helmet", "protective gear", "protection", "protective"), #if no dedicated helmets section, search protective
    "Mouthguards": ("mouthguard", "mouth guard"),
}
...
# EXCLUDED_URL_PATTERNS = ('/brands/', '.html')

def get_path_segments(url: str) -> list[str]:
    """Split a URL's path into non-empty segments.
    e.g. https://fasttimes.com.au/skateboards/protective-gear -> ['skateboards', 'protective-gear']
    """
    path = urlparse(url).path
    return [seg for seg in path.split('/') if seg]


def pick_closest_match(urls: list[str], keywords: tuple[str, ...]) -> str | None:
    """Among candidate URLs, pick the one where a keyword appears as the
    LAST path segment (nothing after it), preferring the shallowest path
    (fewest segments) when multiple qualify."""
    best_url = None
    best_depth = None
    ...
    for url in urls:
        ...
        if any(kw in last_segment for kw in keywords):
            depth = len(segments)
...
def find_nav_links_for_keywords(jina_full_webpage: str) -> dict[str, str | None]:
    category_matches: dict[str, list[str]] = {cat: [] for cat in ENTIRE_PRODUCT_SEARCH_KEYWORDS}
    seen_per_category: dict[str, set[str]] = {cat: set() for cat in ENTIRE_PRODUCT_SEARCH_KEYWORDS}

    for text, url in MARKDOWN_LINK_RE.findall(jina_full_webpage):
        ...
        for category, keywords in ENTIRE_PRODUCT_SEARCH_KEYWORDS.items():
            if any(keyword in text_lower for keyword in keywords):
                if url not in seen_per_category[category]:
                    category_matches[category].append(url)
                    seen_per_category[category].add(url)
            

    best_links: dict[str, str | None] = {}
    for category, urls in category_matches.items():
        keywords = ENTIRE_PRODUCT_SEARCH_KEYWORDS[category]
        best_links[category] = pick_closest_match(urls, keywords)
...
# def find_nav_links_for_keywords(markdown: str, keywords: tuple[str, ...]) -> list[str]: #upto here
#     seen: set[str] = set()
#     matches: list[str] = []
...
# def find_nav_links_for_products(markdown: str, product_categories_sold: set[str],) ->  dict[str, list[str]]:
#     """For each product category in product_categories_sold, look up its
#     keywords and find matching nav links. Returns {category: [urls]}."""
#     results: dict[str, list[str]] = {}
#     for category in product_categories_sold:
#         keywords = PRODUCT_SEARCH_KEYWORDS.get(category, ())
#         if not keywords:
#             continue
#         results[category] = find_nav_links_for_keywords(markdown, keywords)
#     return results
...
async def scan_location(
    ...
    async with httpx.AsyncClient() as client:
        ...
        anchor_website = anchor.get("websiteUri")
        ...
        product_categories_sold = product_stock.copy()
        ...
        #Website analysis part UPTO HERE


        #1 get link of website, variable is -> anchor_website <-
        #2 for each product_category in product_categories_sold
            #3 if there's a NAV link, search for the product_category in NAV links
                #4 click into it and return jina scan of page
            #5 if NO NAV link, search the Search Bar for the product_category
                #return jina scan of page

        

        first_web_scan = await jina_fetch_website_markdown(client, anchor_website)


        category_nav_links = find_nav_links_for_keywords(first_web_scan)



        # category_nav_links = find_nav_links_for_products(first_web_scan, product_categories_sold) #upto here


        testLinks = ""

        for category, url in category_nav_links.items():
            print(url)
        ...
        #upto here, claude scan + prompt from claude chat and extra requirements it gave
